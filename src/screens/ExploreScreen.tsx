...
import { getMealCategory, formatMealTiming } from '../lib/mealUtils';
...
const DIET_OPTIONS: { label: string; icon: string }[] = [
...
export default function ExploreScreen({
  ...
  const toggleLike = useCallback(async (chefId: string) => {
  ...
  const handleReportMeal = useCallback((meal: Meal) => setReportingMeal(meal), []);
      ...
      <main ref={(el) => { scrollRef.current = el; }} className="flex-1 overflow-y-auto hide-scrollbar px-6 py-4 space-y-5 pb-28 relative">
        ...
        {filteredMeals.map((meal) => (
          <MealCard
            ...
            isLiked={meal.host_id ? likedIds.has(meal.host_id) : false}
            isBooked={bookedMeals.has(meal.id)}
            onOpen={handleOpenDetail}
            onBook={handleBookMeal}
            onReport={handleReportMeal}
